...
import com.macro.mall.ums.model.UmsIntegrationConsumeSetting;
import com.macro.mall.ums.model.UmsMember;
import com.macro.mall.ums.model.UmsMemberReceiveAddress;
import com.macro.mall.ums.service.UmsIntegrationConsumeSettingRepository;
...
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
    ...
    public ConfirmOrderResult generateConfirmOrder(List<Long> cartIds) {
        ...
        //获取积分使用规则
        UmsIntegrationConsumeSetting integrationConsumeSetting = integrationConsumeSettingRepository.getById(1L);
    ...
    public Map<String, Object> generateOrder(OrderParam orderParam) {
        ...
        //设置自动收货天数
        List<OmsOrderSetting> orderSettings = orderSettingRepository.list();
        ...
        orderItemRepository.saveBatch(orderItemList);
        //如使用优惠券更新优惠券使用状态
        if (orderParam.getCouponId() != null) {
    ...
    public Integer paySuccess(Long orderId, Integer payType) {
        ...
        OmsOrderDetailPortal orderDetail = orderRepository.getDetailPortal(orderId);
        int count = skuStockRepository.updateSkuStock(orderDetail.getOrderItemList());
        return count;
    ...
    public Integer cancelTimeOutOrder() {
        Integer count=0;
        OmsOrderSetting orderSetting = orderSettingRepository.getById(1L);
        //查询超时、未支付的订单及订单详情
        List<OmsOrderDetailPortal> timeOutOrders = orderRepository.getTimeOutOrders(orderSetting.getNormalOrderOvertime());
        if (CollectionUtils.isEmpty(timeOutOrders)) {
            return count;
        }
        //修改订单状态为交易取消
        List<Long> ids = new ArrayList<>();
        for (OmsOrderDetailPortal timeOutOrder : timeOutOrders) {
            ids.add(timeOutOrder.getId());
        }
        LambdaUpdateWrapper<OmsOrder> lambdaUpdateWrapper = new LambdaUpdateWrapper<OmsOrder>();
        lambdaUpdateWrapper.in(OmsOrder::getId, ids);
        lambdaUpdateWrapper.set(OmsOrder::getStatus, 4);
        orderRepository.update(lambdaUpdateWrapper);
        for (OmsOrderDetailPortal timeOutOrder : timeOutOrders) {
            //解除订单商品库存锁定
            skuStockRepository.releaseSkuStockLock(timeOutOrder.getOrderItemList());
            //修改优惠券使用状态
    ...
    public void cancelOrder(Long orderId) {
        ...
        lambda.eq(OmsOrder::getId, orderId)
                .eq(OmsOrder::getStatus, 0)
                .eq(OmsOrder::getDeleteStatus, 0);
        OmsOrder cancelOrder = orderRepository.getOne(lambda);

        if (cancelOrder != null) {
            //修改订单状态为取消
            cancelOrder.setStatus(4);
            orderRepository.updateById(cancelOrder);
            LambdaQueryWrapper<OmsOrderItem> lambdaItem = new LambdaQueryWrapper<>();
            lambdaItem.eq(OmsOrderItem::getOrderId, orderId);
            List<OmsOrderItem> orderItemList = orderItemRepository.list(lambdaItem);
            //解除订单商品库存锁定
            if (!CollectionUtils.isEmpty(orderItemList)) {
                skuStockRepository.releaseSkuStockLock(orderItemList);
            }
            //修改优惠券使用状态
    ...
    public void confirmReceiveOrder(Long orderId) {
        ...
        OmsOrder order = orderRepository.getById(orderId);
    ...
    @Override
    public CommonPage<OmsOrderDetailPortal> list(Integer status, Integer pageNum, Integer pageSize) {
        if(status==-1){
            status = null;
        }
        UmsMember member = memberService.getCurrentMember();
    ...
    @Override
    public OmsOrderDetailPortal detail(Long orderId) {
        OmsOrder omsOrder = orderRepository.getById(orderId);
        LambdaQueryWrapper<OmsOrderItem> lambda = new LambdaQueryWrapper<>();
        lambda.eq(OmsOrderItem::getOrderId, orderId);
        List<OmsOrderItem> orderItemList = orderItemRepository.list(lambda);
        OmsOrderDetailPortal orderDetail = new OmsOrderDetailPortal();
        BeanUtil.copyProperties(omsOrder,orderDetail);
    ...
    private void updateCouponStatus(Long couponId, Long memberId, Integer useStatus) {
        ...
        List<SmsCouponHistory> couponHistoryList = couponHistoryRepository.list(lambda);

        if (!CollectionUtils.isEmpty(couponHistoryList)) {
            SmsCouponHistory couponHistory = couponHistoryList.get(0);
            couponHistory.setUseTime(new Date());
            couponHistory.setUseStatus(useStatus);
            couponHistoryRepository.updateById(couponHistory);
        }
    }
    ...
    private void lockStock(List<CartPromotionItem> cartPromotionItemList) {
        for (CartPromotionItem cartPromotionItem : cartPromotionItemList) {
            PmsSkuStock skuStock = skuStockRepository.getById(cartPromotionItem.getProductSkuId());
            skuStock.setLockStock(skuStock.getLockStock() + cartPromotionItem.getQuantity());
            skuStockRepository.updateById(skuStock);
        }
    }
